...
const createUnitTitle = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        res.status(statusCode.CREATED).send(util.success(statusCode.CREATED, message.CREATE_UNIT_TITLE_SUCCESS, data));
    } catch (error) {
...
const updateUnitTitle = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const data = await unitTitleService.updateUnitTitle(unitTitleId, unitTitleUpdateDto);
        res.status(statusCode.CREATED).send(util.success(statusCode.OK, message.UPDATE_UNIT_TITLE_SUCCESS, data));
    } catch (error) {
        res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, message.INTERNAL_SERVER_ERROR));
    }
}

const findUnitTitleById = async (req: Request, res: Response): Promise<void> => {
    const { unitTitleId } = req.params;
...
const deleteUnitTitle = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        res.status(statusCode.CREATED).send(util.success(statusCode.OK, message.DELETE_UNIT_TITLE_SUCCESS, data));
    } catch (error) {
        res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, message.INTERNAL_SERVER_ERROR));
    }
}
